...
## 独立 scientific-figure-making Skill

仓库内另行安装了可独立触发的 `skills/scientific-figure-making/SKILL.md`，用于出版级 Matplotlib 图件。其上游正文与五份引用保留在独立目录并继续受 CC BY-NC 4.0 约束，不因本仓库根 MIT License 而重新许可。调用该 Skill 时，仍必须先服从本文件的 Revalidation/Grounding、本仓 `references/figure-contract.md` 与 `references/figures4papers-profile.md`；具体来源、改动与许可边界见 `skills/scientific-figure-making/SOURCE.md`。
