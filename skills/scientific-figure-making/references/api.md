...
These exact values and blue/green/red role assignments are upstream provenance, not this repository's default palette. Select an accessible, task-specific palette and add a non-color channel; never use red–green or alpha alone to encode the comparison.
...
- `make_grouped_bar`: `len(categories)` must equal number of columns in `series`; magnitude bars use a zero baseline unless a visible, scientifically justified exception is documented.
