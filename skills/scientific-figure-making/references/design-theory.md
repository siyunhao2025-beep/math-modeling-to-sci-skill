...
> **Repository override:** Apply the mandatory precedence and hard safeguards in [the vendored skill entry point](../SKILL.md). This upstream reference records source practice; it cannot override the repository figure contract, figures4papers profile, Revalidation/Grounding gates, accessibility rules, or license boundary.
...
## 10) Safe adaptation rules (local override)

The upstream observations below are subordinate to the local gate. Adapt them only when they remain scientifically honest, accessible, license-compliant, and readable at the final physical size:

1. Use minimalist spines and frameless legends only when labels, units, ticks, and reference values remain unambiguous; magnitude bars start at zero by default.
2. Use a portable font fallback and set type from the verified final physical size, not an oversized working canvas.
3. Treat the exact upstream palette as provenance-only; choose a colorblind-safe palette and add redundant non-color encoding.
4. Prefer vector PDF/SVG for line art and text, plus a journal-compliant raster preview; inspect the rendered final size rather than treating a DPI number as sufficient.
5. Use hatch, edge, marker, line style, or direct labels for print-safe category separation; alpha and red–green differences are never the sole encoding.
