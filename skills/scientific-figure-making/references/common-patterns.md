...
## Ultra-wide aspect for multi-metric panels — upstream observation, not a local default

For 3-4 metrics or many categories in a single row, use a wide canvas so bars/labels don't crowd vertically.

- **Upstream examples:** `figsize=(45, 12)` or `(28, 6)`. Under the local gate, do not copy these fixed sizes; derive the canvas from the verified final physical width and split or wrap panels when necessary.
...
## Categorical bars without x-tick labels — conditional only

When the x-axis is "method" or "condition" and the legend already identifies them, hide x ticks and rely on the legend/title.

- **Upstream code pattern:** `ax.set_xticks([])` or empty tick labels. Under the local gate, use it only when direct labels or an equally precise mapping keeps every mark unambiguous.
- **Use when:** Many methods across multiple metrics; names are in the legend or panel title.

---

## Dynamic y-axis scaling — forbidden for magnitude bars
...
- **Local restriction:** Magnitude bars start at zero by default. A non-bar axis may be tightened only when the range is explicit, the effect is not exaggerated, and the caption explains the choice.
